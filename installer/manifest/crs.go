...
func (g CRSGenerator) Generate(ctx context.Context, m *manifestGenerator) ([]Resource, error) {
	...
	}), retry.OnlyRetryableErrors(), retry.WithExponentialBackoff())
...
func (g *CRSGenerator) centralConnection(ctx context.Context, port int, m *manifestGenerator) (*grpc.ClientConn, error) {
	...
	centralHostPort := fmt.Sprintf("central.%s.svc:443", m.Config.Namespace)
	if m.Config.CRS.PortForward {
...
func extractCRS(input []byte) string {
	...
	for scanner.Scan() {
		line := scanner.Text()
		if s, found := strings.CutPrefix(line, "  crs: "); found {
			return s
		}
	}
	return ""
